...
function char_trans_dump(id, rid, trid, newname){
    $.ajax({
        url: '/characters/write_dump',
        data: {id: id, rid: rid},
        dataType: 'json',
        success: function( data ) {
            if(data.status == 'success'){
                show_msg($('#status'), data.msg, data.status);
                char_trans_load(id, rid, trid, newname)
            } else {
                show_msg($('#status'), data.msg, data.status);
            }
        },
        error: function(jqXHR, textStatus){
            show_msg($('#status'), textStatus, 'error');
        }
    });        
    return false;
}

function char_trans_load(id, rid, trid, newname){
    $.ajax({
        url: '/characters/load_dump',
        data: {id: id, rid: rid, trid: trid, newname: newname},
        dataType: 'json',
        success: function( data ) {
            if(data.status == 'success'){
                show_msg($('#status'), data.msg, data.status);
                char_trans_erase(id, rid, trid, newname)
            } else {
                show_msg($('#status'), data.msg, data.status);
            }
        },
...
function char_trans_erase(id, rid, trid, newname){
    $.ajax({
        url: '/characters/erase',
        data: {id: id, rid: rid},
        dataType: 'json',
        success: function( data ) {
            if(data.status == 'success'){
