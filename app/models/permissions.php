...
class Permissions {
    ...
    public static function check_permission($controller,$action,$role='guest'){
        ...
        Logger::debug("Checking permission with '$role' on '$controller / $action'");
        
        if(!isset(self::$roles[$role])){
            self::$roles[$role] = self::load_role_permissions($role);
            Logger::debug(self::$roles[$role]);
        }
        ...
        if(is_null($allowed)) {
            ...
            Logger::debug("Couldn't find permission falling back to default => ".var_export($allowed,true));
        }
    ...
    private static function parse_permissions($perms,$child=false){
        ...
        if(is_array($perms) && $child && isset($perms[$child])){
            $allowed = $perms[$child];
            Logger::debug("Found permission on action-level => ".var_export($allowed,true));
        } elseif(is_bool($perms) || is_numeric($perms)) {
            $allowed = $perms;
            Logger::debug("Found permission on controller-level => ".var_export($allowed,true));
        }
        return $allowed;
    }
}
