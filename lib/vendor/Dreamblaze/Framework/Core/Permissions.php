...
class Permissions {
    ...
    public static function check_permission($controller,$action,$role='guest'){
        ...
        Logger::debug("Checking permission on '$role' for '$param_controller / $action'", $controller);

        if(!method_exists($controller,$action)){
            Logger::debug("Action $action not found", $controller);
            return false;
        }
        ...
        if(!is_null($perms)){
            ...
            if($allowed == null){
                ...
                Logger::debug('No permission found! Falling back to default: ' . var_export($allowed, true), $controller);
            }
    ...
    private static function parse_permission_set($controller_perms,$action){
        if(is_array($controller_perms) && $action && isset($controller_perms[$action])){
            Logger::debug("Found permission on action-level", $action);
            return $controller_perms[$action];
        } elseif(is_bool($controller_perms) || is_numeric($controller_perms)) {
            Logger::debug("Found permission on controller-level", $action);
            return $controller_perms;
        } else {
            Logger::debug("Couldn't find permission", $action);
            return null;
        }
    }
}
